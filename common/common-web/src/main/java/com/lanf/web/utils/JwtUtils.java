...
public class JwtUtils {
     ...
     * @param deviceId 设备ID
     * @param tenantId 租户ID
     * @param expTime 过期时间 分钟
     * @return JWT Token
     */
    public static String createTokenForAdminWithMinutes(Long userId, String deviceId, Long tenantId, long expTime) {
    ...
    public static JwtTokenInfo parseAdminToken(String token) throws IExpiredJwtException, TokenParseException {
        try {
            ...
            JwtTokenInfo tokenInfo = new JwtTokenInfo();
            tokenInfo.setUserId(Long.parseLong(claims.get(JwtUtils.CLAIM_USER_ID,String.class)));
            tokenInfo.setDeviceId(claims.get(CLAIM_DEVICE_ID, String.class));
